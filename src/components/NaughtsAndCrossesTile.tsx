...
const NaughtsAndCrossesTile:React.FC<tileProps> = ({ value, onTileClick, done }) => {
    return (<Button variant="alternative" additionalClasses={`transition-all w-12 h-12 text-sm font-medium ${ done ? 'text-rose-700' :  'text-gray-900' }  ${ done ? 'dark:text-rose-500' :  'dark:text-gray-400' }`} onClick={ onTileClick }>
        {
            (value === "X" && 
            ...
            (value === "O" &&
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
